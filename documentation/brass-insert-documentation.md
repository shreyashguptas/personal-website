...
The library generates dimensionally accurate models of this style across the entire M2–M6 range, with every feature parameterized so a single file can be reused for all 18 common sizes.
...
The 18 sizes above are the ones physically measured and tested from that kit. The practical limit of the library is the pitch lookup (see [The Pitch Formula](#the-pitch-formula)), which covers M2–M6 coarse threads — any standard double-knurled brass insert in that thread range can be generated by changing the three inputs.
